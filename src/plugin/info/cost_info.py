...
주요 기능:
- cost_info: 단일 비용 데이터 변환
- costs_info: 여러 비용 데이터 변환
- linked_accounts_info: 연결된 계정 정보 변환
"""
...
# 비용 데이터 변환 함수
__all__ = ["cost_info", "costs_info", "linked_accounts_info"]
# 로깅 설정
_LOGGER = logging.getLogger(__name__)


def cost_info(cost_data):
    """
    ...
    try:
        ...
        # 디버깅을 위한 로그 출력
        _LOGGER.debug(f"[cost_info] error reason: {e}", exc_info=True)
        raise e


def costs_info(costs_data, **kwargs):
    """
    여러 건의 비용 데이터를 SpaceONE CostsInfo 프로토버퍼 객체로 변환합니다.

    동작 방식:
        - 입력 데이터가 리스트이거나, 딕셔너리 내 "results" 키에 리스트로 포함된 경우 모두 지원합니다.
        - 각 비용 데이터는 cost_info 함수로 개별 변환됩니다.
        - 변환된 결과 리스트를 results 필드에 담아 costs_info 객체로 반환합니다.

    Args:
        costs_data (list 또는 dict): 변환할 비용 데이터 리스트 또는 {"results": [...]} 형태의 딕셔너리
        **kwargs: cost_info 함수에 전달할 추가 매개변수
    ...
    try:
        ...
        # 각 비용 데이터를 cost_info로 변환하여 리스트로 만든 후 CostsInfo 객체 생성
        results = []  # 변환된 비용 데이터 리스트
        for cost_data in costs_data:
            if isinstance(cost_data, dict):  # 비용 데이터가 딕셔너리인 경우
                results.append(cost_info(cost_data))
            else:
                _LOGGER.warning(
                    f"[costs_info] Invalid cost_data type: {type(cost_data)}, value: {cost_data}"
                )  # 비용 데이터가 딕셔너리가 아닌 경우 경고 메시지 출력

        return cost_pb2.CostsInfo(results=results)
    except Exception as e:
        _LOGGER.error(f"[costs_info] error reason: {e}", exc_info=True)
        raise e


def linked_accounts_info(linked_accounts_data, **kwargs):
    """
    ...
    try:
        ...
        # 디버깅을 위한 로그 출력
        _LOGGER.debug(f"[linked_accounts_info] error reason: {e}", exc_info=True)
        raise e

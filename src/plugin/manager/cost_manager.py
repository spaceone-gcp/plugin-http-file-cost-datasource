...
class CostManager(BaseManager):
    ...
    def get_data(
        self, 
        options: Dict[str, Any],  # 플러그인 옵션 (base_url, field_mapper, default_vars, type_mapper 등)
        secret_data: Dict[str, Any],  # 인증 정보
        schema: Optional[str],  # 스키마 정보 (선택)
        task_options: Optional[Dict[str, Any]]  # 작업별 옵션 (base_url 또는 bucket_name)
        ...
        try:
            ...
            response_stream = self._get_data_stream(options, secret_data, schema, task_options)  # 데이터 스트림 생성
            ...
            _LOGGER.info(f"비용 데이터 수집 완료: {total_processed_count}개 처리, {duration:.2f}초 소요")
    ...
    def _get_data_stream(
        ...
        if "base_url" in task_options:  # base_url 옵션이 있으면 HTTP 스트림 생성
            return self._get_http_stream(options, secret_data, schema, task_options["base_url"])
        elif "bucket_name" in task_options:  # bucket_name 옵션이 있으면 Google Cloud Storage 스트림 생성
            ...
            return self._get_http_stream(options, secret_data, schema, options["base_url"])
        else:  # 데이터 소스 정보가 없으면 오류 발생
            raise ValueError("데이터 소스 정보가 필요합니다 (base_url 또는 bucket_name)")
    ...
    def _get_http_stream(
        ...
        Args:
            ...
            base_url: 기본 URL
        """
        http_file_connector = self.locator.get_connector(HTTPFileConnector)  # HTTPFileConnector 인스턴스 생성
    ...
    def _get_storage_stream(
        ...
        storage_connector = self.locator.get_connector(GoogleStorageConnector, secret_data=secret_data)  # GoogleStorageConnector 인스턴스 생성
    ...
    def _process_additional_info_mapping(
        ...
        for origin_field, actual_field in additional_mapping.items():  # 추가 매핑 순회
            if actual_field in result:  # 필드 존재 확인
                additional_info[origin_field] = result[actual_field]  # 추가 매핑 정보 추가
                del result[actual_field]  # 원본 필드 삭제
    ...
    def _process_account_id_mapping(self, result: Dict[str, Any]) -> None:
        ...
        if ("Account ID" in type_mapper_info and  # Account ID 확인
            "additional_info" in result and  # additional_info 필드 확인
            "Account ID" in result.get("additional_info", {})):  # Account ID 확인
            
            account_id = result["additional_info"]["Account ID"]  # Account ID 추출
            if isinstance(account_id, (int, float)):
                result["additional_info"]["Account ID"] = str(account_id).zfill(CostManagerConfig.ACCOUNT_ID_PADDING_LENGTH)  # Account ID 패딩 처리
    ...
    def _process_existing_billed_date(self, result: Dict[str, Any]) -> bool:
        ...
        if result.get("billed_date"):  # billed_date 필드 확인
            ...
            if billed_date is not None:  # billed_date 값 확인
                ...
                result["billed_date"] = parsed_date.strftime(CostManagerConfig.DATE_FORMAT)  # 날짜 형식 변환
            return True
        return False
    ...
    def _process_usage_start_time(self, result: Dict[str, Any]) -> bool:
        ...
        if "usage_start_time" in result:  # usage_start_time 필드 확인
            ...
            result["billed_date"] = self._format_date_only(usage_start_time)  # 날짜 형식 변환
    ...
    def _process_invoice_month(self, result: Dict[str, Any]) -> bool:
        ...
        if invoice_month and len(invoice_month) == CostManagerConfig.INVOICE_MONTH_LENGTH:  # invoice.month 길이 확인
            result["billed_date"] = self._format_invoice_month(invoice_month)  # 날짜 형식 변환
            return True  # 날짜 처리 완료
        return False
    ...
    def _extract_invoice_month(self, result: Dict[str, Any]) -> Optional[str]:
        ...
        # 평면화된 형태
        if "invoice.month" in result and result["invoice.month"] is not None:  # invoice.month 필드 확인
            return str(result["invoice.month"])  # invoice.month 추출
        
        # 딕셔너리 형태
        if ("invoice" in result and 
            isinstance(result["invoice"], dict) and 
            "month" in result["invoice"] and 
            result["invoice"]["month"] is not None):
    ...
    def _format_invoice_month(self, invoice_month: str) -> str:
        ...
        year = invoice_month[CostManagerConfig.YEAR_START_INDEX:CostManagerConfig.YEAR_END_INDEX]  # 연도 추출
        month = invoice_month[CostManagerConfig.MONTH_START_INDEX:CostManagerConfig.MONTH_END_INDEX]  # 월 추출
    ...
    def _process_year_month(self, result: Dict[str, Any]) -> bool:
        ...
        if "year" in result and "month" in result:  # year, month 필드 확인
            year = str(result["year"]) if result["year"] is not None else ""  # year 추출
            month = str(result["month"]) if result["month"] is not None else ""  # month 추출
            day = str(result.get("day", CostManagerConfig.DEFAULT_DAY)) if result.get("day") is not None else CostManagerConfig.DEFAULT_DAY  # day 추출
    ...
    def _format_date_only(date_str: str) -> str:
        ...
        try:
            ...
            if len(date_str) >= 10 and date_str[4] == '-' and date_str[7] == '-':  # 날짜 형식 확인
    ...
    def _create_final_data(self, result: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if result.get("provider") == CostManagerConfig.CSV_PROVIDER:  # provider 확인
            ...
            usage_quantity = Decimal(result.get("usage_quantity") or 0)  # usage_quantity 추출
            region_code = result.get("region_code") or ""  # region_code 추출
            product = result.get("product") or ""  # product 추출
        else:
            credits_amount = Decimal(result.get("credits.amount") or 0)  # credits.amount 추출
            usage_quantity = Decimal(result.get("usage.amount_in_pricing_units") or 0)  # usage.amount_in_pricing_units 추출
    ...
    def get_linked_accounts(
        ...
        try:
            ...
            data_source_type = self._determine_data_source_type(options)  # 데이터 소스 타입 결정
            _LOGGER.debug(f"데이터 소스 타입: {data_source_type}")  # 데이터 소스 타입 로깅
            
            linked_accounts = self._extract_linked_accounts(data_source_type, options)  # 연결된 계정 정보 추출
    ...
    def _extract_linked_accounts(self, data_source_type: str, options: Dict[str, Any]) -> List[Dict[str, str]]:
        ...
        if data_source_type == "http_file":  # HTTP 파일 타입 확인
            ...
            _LOGGER.warning("Google Cloud Storage에서 계정 정보 추출 로직이 구현되지 않음")
        else:

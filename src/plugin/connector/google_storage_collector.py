...
# 베이스 클래스 import
from plugin.connector.base_file_connector import BaseFileConnector, MIN_FILE_SIZE, SUPPORTED_EXTENSIONS  # 베이스 클래스 import
...
class GoogleStorageConnector(BaseFileConnector):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.client = storage.Client(
            project=self.secret_data["project_id"], credentials=self.credentials  # 프로젝트 ID 설정
        )
    ...
    def create_session(self, options: dict, secret_data: dict, schema: str = None) -> None:
        ...
        if secret_data:  
            ...
            self.client = storage.Client(
                project=secret_data["project_id"], credentials=self.credentials  # 프로젝트 ID 설정
            )

    def get_cost_data(self, task_options: dict) -> Generator[List[Dict[str, Any]], None, None]:
        """
        ...
        if not bucket_name:
            raise ERROR_REQUIRED_PARAMETER(key="task_options.bucket_name")  # 파라미터 없는 경우 오류 발생
        ...
        for blob_name in blob_names:
            ...
            if not blob_name.strip() or blob_name.endswith('/'):  # 디렉토리 또는 빈 blob 이름은 건너뜀
                continue  # 건너뜀
            ...
            if blob:
                # 파일 확장자 검증
                file_extension = os.path.splitext(blob_name)[1].lower()  # 파일 확장자 검증
                if file_extension not in SUPPORTED_EXTENSIONS:
                    _LOGGER.warning(f"Skipping unsupported file type: {blob_name} (extension: {file_extension})")
                    continue
                
                # Blob이 비어있는지 미리 확인
                if blob.size == 0:
                    raise ERROR_EMPTY_FILE(file_path=blob_name)  # 파일이 비어있는 경우 오류 발생
                
                # Blob 크기가 너무 작은 경우 경고 (헤더만 있을 수 있음)
                if blob.size < MIN_FILE_SIZE:
                    _LOGGER.warning(f"Blob size is very small: {blob.size} bytes for {blob_name}")  # 파일 크기가 너무 작은 경우 경고
                    _LOGGER.warning("This might indicate an empty or header-only file")  # 파일이 비어있거나 헤더만 있을 수 있음
                
                # 임시 디렉토리 경로 확보
                tmpdir = tempfile.gettempdir()  # 임시 디렉토리 경로 확보
                
                # 안전한 임시 파일명 생성
                safe_filename = self.generate_safe_filename(blob_name)  # 안전한 임시 파일명 생성
                ...
                try:
                    blob.download_to_filename(temp_file_path)  # blob 파일을 임시 파일로 다운로드
                except Exception:
                    raise ERROR_FILE_DOWNLOAD_FAILED(file_path=blob_name)  # 파일 다운로드 실패 시 오류 발생
                ...
                if file_size == 0:
                    ...
                    try:
                        ...
                        _LOGGER.error(f"Failed to read file content: {e}")  # 파일 읽기 실패 시 로깅
                    raise ERROR_EMPTY_FILE(file_path=blob_name)  # 파일이 비어있는 경우 오류 발생
                
                # 파일에서 비용 데이터 읽기 (CSV/JSON/Parquet 자동 판별)
                costs_data = self.parse_cost_file(temp_file_path)  # 파일에서 비용 데이터 읽기
    ...
    def _check_options(options: dict) -> None:
        ...
        if "bucket_name" not in options:  # bucket_name이 없는 경우
            raise ERROR_REQUIRED_PARAMETER(key="options.bucket_name")  # 파라미터 없는 경우 오류 발생

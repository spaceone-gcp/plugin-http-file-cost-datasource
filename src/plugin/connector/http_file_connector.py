...
# 검증 모듈 import
from plugin.validation import PayloadValidationError, PayloadValidator
...
class HTTPFileConnector(BaseFileConnector):
    ...
    def create_session(
        ...
        # 강화된 검증 로직 적용
        try:
            validated_options = PayloadValidator.validate_options(options)
            if secret_data:
                PayloadValidator.validate_secret_data(secret_data)
        except PayloadValidationError as e:
            from spaceone.core.error import ERROR_INVALID_PARAMETER

            raise ERROR_INVALID_PARAMETER(key="options", reason=str(e)) from e

        self.base_url = validated_options.get("base_url")  # 기본 URL 설정

        # base_url이 None인 경우 경고만 로그
        if not self.base_url:
            _LOGGER.warning(
                "[HTTPFileConnector.create_session] base_url이 설정되지 않았습니다."
            )

        if "field_mapper" in validated_options:  # 필드 매핑 설정
            self.field_mapper = validated_options.get("field_mapper")
        if "default_vars" in validated_options:  # 기본 변수 설정
            self.default_vars = validated_options.get("default_vars")
    ...
    def get_cost_data(
        ...
        # URL 검증
        try:
            validated_url = PayloadValidator._validate_single_url(base_url)
        except PayloadValidationError as e:
            from spaceone.core.error import ERROR_INVALID_PARAMETER

            raise ERROR_INVALID_PARAMETER(key="base_url", reason=str(e)) from e

        # 파일 형식에 따른 파싱 메서드 호출
        if self.is_json_file(validated_url):  # JSON 파일 처리
            costs_data = self._get_json(validated_url)  # JSON 파일 파싱
        elif self.is_parquet_file(validated_url):  # Parquet 파일 처리
            costs_data = self._get_parquet(validated_url)  # Parquet 파일 파싱
        else:
            costs_data = self._get_csv(validated_url)  # CSV 파일 파싱
    ...
    def _check_options(options: dict) -> None:
        ...
        if "base_url" not in options:
            _LOGGER.warning(
                "[HTTPFileConnector.check_options] base_url이 options에 없습니다."
            )
            return  # 에러를 발생시키지 않고 리턴
    ...
    def _get_json(self, base_url: str) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            # 임시 파일에 저장
            file_extension = (
                ".json.gz" if base_url.lower().endswith(".json.gz") else ".json"
            )
            with tempfile.NamedTemporaryFile(

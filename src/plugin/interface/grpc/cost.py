...
from plugin.info.cost_info import costs_info, linked_accounts_info
...
class Cost(BaseAPI, cost_pb2_grpc.CostServicer):
    ...
    def get_data(self, request, context):
        ...
        Yields:
            costs_info: 처리된 비용 데이터 정보 객체들의 스트림
        """
        ...
        with self.locator.get_service(CostService, metadata) as cost_service:
            ...
            # 각 비용 데이터를 costs_info 객체로 변환하여 응답 스트림 생성
            for costs_data in response_stream:
                yield self.locator.get_info(costs_info, costs_data)
    ...
    def get_linked_accounts(self, request, context):
        ...
        Returns:
            linked_accounts_info: 연결된 계정 정보 객체
        """
        # gRPC 요청을 파싱하여 파라미터와 메타데이터 추출
        params, metadata = self.parse_request(request, context)
        ...
        with self.locator.get_service(CostService, metadata) as cost_service:
            ...
            # linked_accounts_info 객체로 변환하여 응답
            return self.locator.get_info(linked_accounts_info, linked_accounts_data)

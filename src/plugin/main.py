...
class CostGetDataParams:
    ...
    def __init__(self, params: Dict[str, Any]):  # 파라미터 초기화
        ...
        self.secret_data: Dict[str, Any] = params.get("secret_data", {})  # 시크릿 데이터
        self.schema: Optional[str] = params.get("schema")  # 스키마 정보
        self.task_options: Dict[str, Any] = params.get("task_options", {})  # 작업별 옵션
...
def _validate_and_clean_secret_data(secret_data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        if not hasattr(secret_data, 'get'):  # secret_data가 get 메서드를 가지고 있는지 확인
    ...
    if 'private_key' in secret_data:  # PEM 키 존재 시 정리
        secret_data['private_key'] = _clean_pem(secret_data['private_key'])  # PEM 키 정리 함수 호출
...
def data_source_init(params: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        _LOGGER.info(f"[data_source_init] 데이터 소스 초기화 시작 - domain_id: {domain_id}")
        
        data_source_mgr = DataSourceManager()  # DataSourceManager 인스턴스 생성
        result = data_source_mgr.init_response(options)  # 데이터 소스 초기화 응답 생성
        
        _LOGGER.info(f"[data_source_init] 데이터 소스 초기화 완료 - domain_id: {domain_id}")
...
def data_source_verify(params: Dict[str, Any]) -> None:
    ...
    try:
        ...
        # 4. 데이터 소스 매니저를 통해 검증
        _LOGGER.info(f"[data_source_verify] 데이터 소스 검증 시작 - domain_id: {domain_id}")
        
        data_source_mgr = DataSourceManager()  # DataSourceManager 인스턴스 생성
        data_source_mgr.verify_plugin(options, cleaned_secret_data, domain_id, schema)  # 데이터 소스 검증
        
        _LOGGER.info(f"[data_source_verify] 데이터 소스 검증 완료 - domain_id: {domain_id}")
...
def job_get_tasks(params: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        result = job_mgr.get_tasks(
            domain_id, options, cleaned_secret_data, schema, start, last_synchronized_at  # 작업 태스크 조회
        )
...
def cost_get_data(params: Dict[str, Any]) -> Generator[Dict[str, Any], None, None]:
    ...
    try:
        ...
        _LOGGER.info(f"[cost_get_data] 데이터 수집 시작 - domain_id: {cost_params.domain_id}")
        ...
        _LOGGER.info(f"[cost_get_data] 데이터 수집 완료 - domain_id: {cost_params.domain_id}")
...
def _validate_data_source(options: Dict[str, Any], task_options: Dict[str, Any]) -> None:
    """데이터 소스 설정 검증
    ...
    """
    has_base_url = "base_url" in task_options or "base_url" in options  # base_url 존재 여부 확인
    has_bucket_name = "bucket_name" in task_options  # bucket_name 존재 여부 확인
    
    if not (has_base_url or has_bucket_name):  # base_url 또는 bucket_name 존재 여부 확인
        raise ValueError(
            "데이터 소스 정보가 필요합니다. "
...
def cost_get_linked_accounts(params: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        _LOGGER.info(f"[cost_get_linked_accounts] 연결된 계정 조회 시작 - domain_id: {domain_id}")
        
        cost_manager = _create_cost_manager()  # CostManager 인스턴스 생성
        result = cost_manager.get_linked_accounts(options, cleaned_secret_data, schema)  # 연결된 계정 조회
        
        _LOGGER.info(f"[cost_get_linked_accounts] 연결된 계정 조회 완료 - domain_id: {domain_id}")
        _LOGGER.debug(f"[cost_get_linked_accounts] Result type: {type(result)}")
        _LOGGER.debug(f"[cost_get_linked_accounts] Result length: {len(result) if isinstance(result, list) else 'N/A'}")
        ...
        _LOGGER.error(f"[cost_get_linked_accounts] 예상치 못한 오류: {e}", exc_info=True)
        raise
